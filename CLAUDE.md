...
| 2026-04-27 | **Direct MLP baseline 학습 완료** | runs/yolograsp_v2/v7_direct_mlp_big/.../best.pt | ep74 종료, **best ep58 val_grasp=0.1118**. patience 30 자연 종료 |
| 2026-04-27 | **Fig 1/2/3 + Table 1 모두 완료** | paper_figs/ | Fig 1: RGB-색칠 PC + 진청 grasp + 35% 작은 gripper, elev=-15 azim=205. Fig 2: 단일행 mathtext, 신경망 깊이 비공개. Fig 3: **seen 씬** (random4_56 / random1_39 / random4_41), N=16, CFG=2.5, mode별 dist filter, standing top:side=12:4 balance, computed_zorder=False 로 grasp 앞 표시. Table 1: COV+APD+MAE per-mode |
| 2026-04-27 | **공식 metric 정립 (COV + APD)** | scripts/make_paper_table1.py + 메모리 paper_metrics_official.md | Achlioptas ICML 2018 (COV) + Sundermeyer ICRA 2021 (5cm/30° threshold). standing 3 모드 COV: Direct **33.3%** vs Ours **99.2%**, 전체 APD: 0.22 → 4.32 cm (**20×**). bibitem 포함 |
| 2026-04-27 | **모델 업그레이드 v8 분석** | 메모리 model_upgrade_v8_plan.md | v7 단순 concat 한계 진단 (cross-attn 부재, spatial pooling 손실). 5 옵션 ROI 비교: A(cross-attn) + C(multi-scale crop) 권장. 트리거 조건 미충족 시 v7 유지 |
| 2026-04-27 | **시뮬레이션 환경 표기 확정** | 메모리 research_paper_plan.md | "MATLAB R2025b + Simulink 3D Animation Toolbox (Unreal Engine renderer)", scene-level split (random1-5 train, random6 val) 명시 |
