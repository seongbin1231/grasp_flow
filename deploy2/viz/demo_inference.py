...
def add_scene(fig, row, col, depth, uv_px, obj_mask, grasps_8d, flags,
    ...
    n_coll = int(sum(1 for f in flags if f == "body_collision"))
    n_air = int(sum(1 for f in flags if f == "no_contact"))

    # Filtered grasps: draw FIRST (underneath), faded
    if filt_mask.any():
        _add_grasp_group(
            fig, row, col, grasps_8d[filt_mask],
            color="rgba(160,160,160,0.55)", width_main=2.0, dash_main="dot",
            legendgroup="filtered",
            name=f"filtered ({n_coll}c / {n_air}a)",
            showlegend=show_legend, show_cone=False,
        )

    # Kept grasps: red, solid, on top
    if kept_mask.any():
        _add_grasp_group(
            fig, row, col, grasps_8d[kept_mask],
            color="#e41a1c", width_main=4.0, dash_main="solid",
            legendgroup="kept",
    ...
    with h5py.File(POSES, "r") as poses, h5py.File(YOLO_CACHE, "r") as dets:
        for ci, (sid, oname) in enumerate(pairs):
            ...
            print(f"[{cat_name}] {sid}/{oname} cls={cls} "
                  ...
                  f"collision={n_coll}  no_contact={n_air}{extra}")
