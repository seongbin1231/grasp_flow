# YOLO_Grasp Flow Matching — MATLAB 통합 스펙 (v7_v4policy)
...
체크포인트: `v7_v4policy_big/best.pt`, ep=226/250, val_flow=**0.3676**
아키텍처: AdaLN-Zero × 12 blocks, hidden=1024, params=**35.28M** (v6 대비 +36%)
정책: **6dof-v4** (lying tilt ±30°/±15°, standing 0°/45°/90° 3-layer, spam cap-up only)
내보낸 ONNX: [encoder.onnx](encoder.onnx) (2.38 MB), [velocity.onnx](velocity.onnx) (**139.22 MB**), [meta.json](meta.json)
수치 검증: onnxruntime round-trip max|Δ| = 5.96e-06 (float32 허용 범위).
...
| `velocity.onnx` | **139 MB** | g_t, cond, t, uv_norm → v (8-d) | **T_steps × 2회** (CFG on/off) |
| `meta.json` | ~2 KB | pos 정규화 통계, 정책 메타, 추천 하이퍼 | (로드시 1회) |
